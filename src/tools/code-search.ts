...
export const codeSearchTool: ToolDefinition = {
  ...
  handler: async (params: CodeSearchParams, context: ToolContext): Promise<ToolResult> => {
    try {
...
function shouldIgnoreConfigurable(filename: string, relativePath: string, ignorePatterns: string[]): boolean {
...
function shouldIgnore(filename: string, relativePath: string): boolean {
  const ignoredDirs = [
    'node_modules',
    '.git',
    '.vscode',
...
function extractContextFlag(flags: string, flag: string): number | null {
