...
export class ProjectContextDetector {
  ...
  static detectProjectRoot(startPath: string = process.cwd()): string {
  ...
  private static detectProjectType(rootPath: string): ProjectType {
  ...
  private static detectProjectName(rootPath: string, type: ProjectType): string {
  ...
  private static detectLanguages(rootPath: string): string[] {
  ...
  private static detectFrameworks(rootPath: string): string[] {
  ...
  private static detectPackageManager(rootPath: string): string | null {
    const files = this.getDirectoryContents(rootPath)
  ...
  private static analyzeStructure(rootPath: string): ProjectStructure {
  ...
  private static isConfigFile(name: string): boolean {
    const configs = [
  ...
  private static hasReactDependencies(rootPath: string): boolean {
    try {
      const packageJson = JSON.parse(readFileSync(join(rootPath, 'package.json'), 'utf8'))
      const deps = { ...packageJson.dependencies, ...packageJson.devDependencies }
  ...
  private static hasVueDependencies(rootPath: string): boolean {
    try {
      const packageJson = JSON.parse(readFileSync(join(rootPath, 'package.json'), 'utf8'))
      const deps = { ...packageJson.dependencies, ...packageJson.devDependencies }
  ...
  private static getDirectoryContents(path: string): string[] {
    try {
