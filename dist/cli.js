...
export class InteractiveCLI {
    ...
    async initializeClient() {
        ...
        try {
            ...
            this.agentSystem = new EnhancedAgentSystem(this.client, {
    ...
    async handleTestTool(args) {
        ...
        try {
            ...
            if (this.agentSystem) {
                const result = await this.agentSystem.executeTool(toolName, params);
                console.log(`\n${bold('Result:')}`);
    ...
    async showModels() {
        ...
        try {
            ...
            const popular = models.filter((m) => m.id.includes('claude') || m.id.includes('gpt') || m.id.includes('gemini')).slice(0, 10);
    ...
    async handleChat(input) {
        ...
        try {
            ...
            const baseAgent = this.agentSystem.getToolAgent('base');
            if (!baseAgent) {
